<?php
/**
 * openTrans Orderchange Document Writer standard 2.1
 *
...
/**
 * Class to create Orderchange document for CHECK24
 */
class OpentransDocumentWriterOrderchange extends OpentransDocumentWriter
{

    /**
     * Creates and saves xml file from orderchange document
     *
     * @param $src
     * @return false|string
     * @throws OpentransException
     */
    public function get_document_data_orderchange($src)
    {

        if (!$src instanceof OpentransDocumentOrderchange) {
            throw new OpentransException('$src must be type of OpentransDocumentOrderchange.');
        }

        // start with order list, which could contain more then one order
        $xml = new \SimpleXMLElement('<ORDERCHANGE xmlns="http://www.opentrans.org/XMLSchema/2.1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.opentrans.org/XMLSchema/2.1 https://merchantcenter.check24.de/sdk/opentrans/schema-definitions/opentrans_2_1.xsd http://www.bmecat.org/bmecat/2005 https://merchantcenter.check24.de/sdk/opentrans/schema-definitions/bmecat_2005.xsd http://www.w3.org/2005/05/xmlmime https://merchantcenter.check24.de/sdk/opentrans/schema-definitions/xmlmime.xsd http://www.w3.org/2000/09/xmldsig# https://merchantcenter.check24.de/sdk/opentrans/schema-definitions/xmldsig-core-schema.xsd" xmlns:bmecat="http://www.bmecat.org/bmecat/2005" xmlns:xmime="http://www.w3.org/2005/05/xmlmime"></ORDERCHANGE>');

        // Document
        $xml->addAttribute('version', '2.1');

        // Header
        $header = $xml->addChild('ORDERCHANGE_HEADER');

        $NAMESPACE_BMECAT = OpentransConfig::NAMESPACE_BMECAT_URL;

        // Order info
        $oinfo = $header->addChild('ORDERCHANGE_INFO');
        $oinfo->addChild('ORDER_ID', OpentransHelper::formatString($src->get_header()->get_orderchangeinfo()->get_order_id(), 7));
        $oinfo->addChild('ORDERCHANGE_DATE', $src->get_header()->get_orderchangeinfo()->get_orderchange_date());
        if ($sOrderDate = $src->get_header()->get_orderchangeinfo()->get_order_date()) {
            $oinfo->addChild('ORDER_DATE', $sOrderDate);
        }

        $oDb = DatabaseProvider::getDb();
        $orderChangeSequenceId =  (int)$oDb->getOne(
            "select max(sequenceid) from itrcheck24_orderchangerequest where orderid = ?",
            [$src->get_header()->get_orderchangeinfo()->get_order_id()]
        );

        $oinfo->addChild('ORDERCHANGE_SEQUENCE_ID', $orderChangeSequenceId + 1);

        // Order Parties
        $parties = $oinfo->addChild('PARTIES');
        $src_parties = $src->get_header()->get_orderchangeinfo()->get_parties();
        ...
        $src_parties_reference_buyer_idref = OpentransHelper::formatString($src->get_header()->get_orderchangeinfo()->get_idref(OpentransDocumentIdref::TYPE_DELIVERY_IDREF), 46);
        $src_parties_reference_supplier_idref = $src->get_header()->get_orderchangeinfo()->get_idref(OpentransDocumentIdref::TYPE_SUPPLIER_IDREF);
        //$src_parties_reference_invoice_recipient_idref = $src->get_header()->get_orderchangeinfo()->get_idref(OpentransDocumentIdref::TYPE_INVOICE_RECIPIENT_IDREF);
        ...
        // Remarks
        $src_remarks = $src->get_header()->get_orderchangeinfo()->get_remarks();
        ...
        $items = $xml->addChild('ORDERCHANGE_ITEM_LIST');
        ...
        for ($i = 0, $i_max = count($src_items); $i < $i_max; ++$i) {
            ...
            $order_reference->addChild('ORDER_ID', OpentransHelper::formatString($src->get_header()->get_orderchangeinfo()->get_order_id(), 7));
        ...
        // Order Summary
        $summary = $xml->addChild('ORDERCHANGE_SUMMARY');
