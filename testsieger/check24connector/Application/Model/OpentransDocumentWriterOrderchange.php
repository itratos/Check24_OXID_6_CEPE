...
class OpentransDocumentWriterOrderchange extends OpentransDocumentWriter
{
    ...
    public function get_document_data_orderchange($src)
    {
        ...
        $oinfo->addChild('ORDER_ID',
            OpentransHelper::formatString($src->get_header()->get_orderchangeinfo()->get_order_id(), 7)
        );
        ...
        for ($i = 0, $i_max = count($src_parties); $i < $i_max; ++$i) {
            ...
            $party_id = $party->addChild('PARTY_ID',
                OpentransHelper::formatString($src_parties[$i]->get_id()->get_id(), 46), $NAMESPACE_BMECAT
            );
        ...
        $src_parties_reference_buyer_idref = OpentransHelper::formatString(
            $src->get_header()->get_orderchangeinfo()->get_idref(OpentransDocumentIdref::TYPE_DELIVERY_IDREF), 46
        );
        ...
        for ($i = 0, $i_max = count($src_items); $i < $i_max; ++$i) {
            ...
            $order_reference->addChild('ORDER_ID',
                OpentransHelper::formatString($src->get_header()->get_orderchangeinfo()->get_order_id(), 7)
            );
