...
<br><br>
                ...
                <legend>Aktionen</legend>
                    ...
                    <input type="submit" class="edittext" id="importOrderChangeButton" value="Orderchange jetzt importieren"
                    onclick="window.open('[{$iframeurl_changeorder}]', 'Orderchange-Import');">
                </fieldset>
                </td>

                <!-- Orderchange request list-->
                <td valign="top" class="edittext" align="left" style="width:100%;height:99%;padding-left:25px;padding-bottom:30px;padding-top:10px;">
                     <h2>[{oxmultilang ident="TSCHECK24CONNECTOR_ORDERCHANGE_REQUEST_BLOCK_TITLE"}]</h2>
                     [{assign var="aCancellationRequests" value=$oView->getPendingCancellationRequests()}]
                     <table>
                            <thead>
                                <tr>
                                    <td>[{oxmultilang ident="TSCHECK24CONNECTOR_ORDERCHANGE_REQUEST_ORDERID"}]</td>
                                    <td>[{oxmultilang ident="TSCHECK24CONNECTOR_ORDERCHANGE_REQUEST_ORDERNR"}]</td>
                                    <td>[{oxmultilang ident="TSCHECK24CONNECTOR_ORDERCHANGE_REQUEST_ACTION"}]</td>
                                    <td>[{oxmultilang ident="TSCHECK24CONNECTOR_ORDERCHANGE_REQUEST_TIME"}]</td>
                                    <td></td>
                                    <td></td>
                                    <td>[{oxmultilang ident="TSCHECK24CONNECTOR_ORDERCHANGE_REQUEST_DENY_REASON"}]</td>
                                </tr>
                            </thead>
                            <tbody>
                            [{foreach from=$aCancellationRequests item=request}]
                                <tr>
                                    <td>[{$request->itrcheck24_orderchangerequest__orderid->value}]</td>
                                    <td>[{$request->itrcheck24_orderchangerequest__oxordernr->value}]</td>
                                    <td>[{$request->itrcheck24_orderchangerequest__action->value}]</td>
                                    <td>[{$request->itrcheck24_orderchangerequest__oxtimestamp->value}]</td>
                                    <td>
                                        <input type="submit" class="edittext" id="confirm_cancellation"
                                            value="[{oxmultilang ident="TSCHECK24CONNECTOR_ORDERCHANGE_REQUEST_CONFIRM"}]"
                                            onclick="Javascript:document.myedit.cl.value='testsieger_orderimport';document.myedit.fnc.value='confirmCancellation';document.myedit.cancellationid.value='[{$request->itrcheck24_orderchangerequest__orderid->value}]'"
                                        >
                                    </td>
                                    <td>
                                        <input type="submit" class="edittext" id="deny_cancellation"
                                            value="[{oxmultilang ident="TSCHECK24CONNECTOR_ORDERCHANGE_REQUEST_DENY"}]"
                                            onclick="Javascript:document.myedit.cl.value='testsieger_orderimport';document.myedit.fnc.value='denyCancellation';document.myedit.cancellationid.value='[{$request->itrcheck24_orderchangerequest__orderid->value}]'"
                                        >
                                    </td>
                                    <td><input type="text" name="deny_reason[[{$request->itrcheck24_orderchangerequest__orderid->value}]]"></td>
                                </tr>
                            [{/foreach}]
                            </tbody>
                     </table>
                </td>
                <!-- End of Orderchange request list-->
